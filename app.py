...
from datetime import datetime, timedelta  # Importer datetime og timedelta

app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:gokstad@localhost/gruppe_5_bibliotek'
...
def profile():
    ...
    return render_template('profile.html', time_of_day=greeting, fornavn=current_user.fornavn,
                           etternavn=current_user.etternavn)
...
def available_books():
    ...
    available_books = Bøker.query.filter(
        ~Bøker.ISBN.in_(db.session.query(LånteBøker.ISBN).filter_by(Levert=False))).all()
...
def borrow_book():
    ...
    lån_dato = datetime.now().date()
    retur_dato = lån_dato + timedelta(days=30)
...
def available_magazines():
    available_magazines = Tidsskrifter.query.filter(
        ~Tidsskrifter.TidsskriftID.in_(db.session.query(LånteTidsskrifter.TidsskriftID).filter_by(Levert=False))).all()
...
def borrow_magazine():
    ...
    lån_dato = datetime.now().date()
    retur_dato = lån_dato + timedelta(days=30)  # Anta 30 dagers lån

    nytt_lån = LånteTidsskrifter(StudentID=current_user.id, TidsskriftID=tidsskrift_id, LånDato=lån_dato,
                                 ReturDato=retur_dato, Levert=False)
    db.session.add(nytt_lån)
    db.session.commit()
...
class Bøker(db.Model):
    ...
    ratings = relationship('BokRating', back_populates='bok')  # Endret her
...
@app.route('/delete-books', methods=['GET'])
@login_required
def show_delete_books():
    if current_user.role != 'admin':
        flash('Ikke tilgang: Kun administratorer har lov til å gjøre dette.', 'error')
        return redirect(url_for('index'))
    books = Bøker.query.all()
    return render_template('delete_books.html', books=books)


@app.route('/delete-book/<int:isbn>', methods=['POST'])
@login_required
def delete_book(isbn):
    if current_user.role != 'admin':
        flash('Ikke tilgang: Kun administratorer har lov til å gjøre dette.', 'error')
        return redirect(url_for('index'))
    book_to_delete = Bøker.query.filter_by(ISBN=isbn).first()
    if book_to_delete:
        db.session.delete(book_to_delete)
        db.session.commit()
        flash('Boken er slettet.', 'success')
    else:
        flash('Boken finnes ikke.', 'error')
    return redirect(url_for('show_delete_books'))
...
def innlevering():
    if request.method == 'POST':
        ...
        for item_id in selected_items:
            ...
            if loan:
                loan.Levert = True
                loan.ReturDato = datetime.now().date()  # Korrigert bruk av datetime
                db.session.commit()
        return redirect(url_for('profile'))
