    ...
    if request.method == 'POST':
        ...
        for item_id in selected_items:
            if item_id.startswith('bok_'):
                loan = LånteBøker.query.get(item_id.replace('bok_', ''))
            elif item_id.startswith('tidsskrift_'):
                loan = LånteTidsskrifter.query.get(item_id.replace('tidsskrift_', ''))
            if loan:
                loan.Levert = True
                loan.ReturDato = datetime.date.today()  # Setter innleveringsdato til dagens dato
                db.session.commit()
        return redirect(url_for('profile'))
